...
const DriverService = {
  ...
  getDriverById: async (id: string): Promise<ApiResponse<Driver>> => {
    try {
      const response = await api.get<ApiResponse<Driver>>(`${ENDPOINTS.DRIVERS}/${id}`);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  createDriver: async (driverData: CreateDriverData): Promise<ApiResponse<Driver>> => {
    try {
      const response = await api.post<ApiResponse<Driver>>(ENDPOINTS.DRIVERS, driverData);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  deleteDriver: async (id: string): Promise<ApiResponse<void>> => {
    try {
      const response = await api.delete<ApiResponse<void>>(`${ENDPOINTS.DRIVERS}/${id}`);
      return response.data;
    } catch (error) {
      throw error;
    }
