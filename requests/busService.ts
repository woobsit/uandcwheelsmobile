...
const BusService = {
  ...
  getBusById: async (id: string): Promise<ApiResponse<Bus>> => {
    try {
      const response = await api.get<ApiResponse<Bus>>(`${ENDPOINTS.BUSES}/${id}`);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  createBus: async (busData: CreateBusData): Promise<ApiResponse<Bus>> => {
    try {
      const response = await api.post<ApiResponse<Bus>>(ENDPOINTS.BUSES, busData);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  updateBus: async (id: string, updateData: Partial<CreateBusData>): Promise<ApiResponse<Bus>> => {
    try {
      const response = await api.patch<ApiResponse<Bus>>(`${ENDPOINTS.BUSES}/${id}`, updateData);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  deleteBus: async (id: string): Promise<ApiResponse<void>> => {
    try {
      const response = await api.delete<ApiResponse<void>>(`${ENDPOINTS.BUSES}/${id}`);
      return response.data;
    } catch (error) {
      throw error;
    }
