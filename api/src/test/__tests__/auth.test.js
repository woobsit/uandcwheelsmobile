...
const User = require('../../models/user.model'); // Import User model directly
const PasswordResetToken = require('../../models/passwordResetToken.model'); // Import PasswordResetToken model directly
const EmailService = require('../../email/email.service'); // Import the service to spy on it (will be mocked automatically)
...
describe('Auth API Endpoints', () => {
  ...
  test('POST /api/v1/auth/register should create a new user', async () => {
    const response = await request(app).post('/api/v1/auth/register').send(testUser); // Use the defined test user data
  ...
  test('POST /api/v1/auth/login should return a token for a registered user', async () => {
    ...
    await request(app).post('/api/v1/auth/register').send(uniqueUser); // Register the unique user for this login test
  ...
  test('POST /api/v1/auth/register should return 409 for duplicate email', async () => {
    ...
    await request(app).post('/api/v1/auth/register').send(testUser); // Assuming this user was registered in a previous test or beforeEach
  ...
  test('GET /api/v1/auth/verify-email should verify user email', async () => {
    ...
    await request(app).post('/api/v1/auth/register').send(verificationTestUser);
    ...
    const response = await request(app).get(`/api/v1/auth/verify-email?token=${verificationToken}`);
  ...
  test('GET /api/v1/auth/verify-email should return 404 for invalid token', async () => {
    const response = await request(app).get('/api/v1/auth/verify-email?token=invalid-token');
  ...
  test('GET /api/v1/auth/verify-email should return 410 for expired token', async () => {
    // Register a user
    await request(app).post('/api/v1/auth/register').send(verificationTestUser); // Re-using user, it will overwrite the previous entry
    ...
    await userInDb?.update({
      verification_token_expires: new Date(Date.now() - 2 * 60 * 60 * 1000),
    }); // 2 hours ago

    const response = await request(app).get(`/api/v1/auth/verify-email?token=${expiredToken}`);
  ...
  test('POST /api/v1/auth/forgot-password should send reset link for existing user', async () => {
    // Register a user
    await request(app).post('/api/v1/auth/register').send(forgotPasswordTestUser);
    ...
    const tokenRecord = await PasswordResetToken.findOne({
      where: { email: forgotPasswordTestUser.email },
    });
  ...
  test('POST /api/v1/auth/reset-password should reset user password', async () => {
    // 1. Register a user
    await request(app).post('/api/v1/auth/register').send(resetPasswordTestUser);
    ...
    const tokenRecord = await PasswordResetToken.findOne({
      where: { email: resetPasswordTestUser.email },
    });
    expect(tokenRecord).toBeDefined();
    ...
    const deletedTokenRecord = await PasswordResetToken.findOne({
      where: { email: resetPasswordTestUser.email },
    });
